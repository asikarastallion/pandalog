...
| 6   | Missing/invalid/unsupported/out-of-range data is never coerced to zero/healthy/default.                                                      | A missing sensor reading is not "0 m/s".                                       | `Validity` is mandatory on every `Sample`; schema validator rejects `VALID` samples with `NaN` and non-`VALID` samples with a finite value. |
...
| 10  | AI explains/summarizes/correlates/hypothesizes; it never invents measurements, timestamps, severity, pass/fail, or root cause.               | Keeps AI subordinate to deterministic analysis.                                | `packages/ai` type contract (`AiAnswer`) has no field that overrides a `VerificationOutcome` or fabricates a `Finding`.                     |
